...
public :
			   ...
			   const vector<int> &constraint_points, const vector<int> &handle_points)
	{
		...
		neighborsOfVertices = Find_Neighbors(nVertices, F);
		...
		cellRotationMatrices = vector<Matrix3d>(nVertices);
	}
	...
	vector<vector<int>> Find_Neighbors(int nV, const MatrixXi &F) 
	{	
...
private :
	...
	static SparseLU<SparseMatrix<double>> solver;
	//LLT<SparseMatrix<double>> solver;
	...
	// Set handle vertices
	void setHandleVertices(const MatrixXd &handle_positions, const vector<int> &handle_points)
	{
	...
	void set_cellRotations(const MatrixXd &V, const MatrixXd &V_prime, const SparseMatrix<double> &W)
	{
		for (int i = 0; i < nVertices; i++) 
		{
			...
			cellRotationMatrices.at(i) = rotate;
		}
	}
	...
	MatrixXd estimateNewPositionOfVertices(const SparseLU<SparseMatrix<double>> &solver, const MatrixXd &b) 
	{
	...
	{
		setHandleVertices(handle_points_positions, handlePts);
